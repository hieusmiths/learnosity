...
function convertRequestPacketToObject(requestPacket) {
  if (lodash.isString(requestPacket)) {
...
const sdkMeta = {
  version: "v0.5.0",
  lang: "node.js",
  lang_version: "v16.16.0",
  platform: "linux",
  platform_version: "5.19.0-35-generic",
};
...
function hashSignatureArray(signatureArray) {
  const hash = sha256(signatureArray.join("_"));

  return hash.toString(); //.digest('hex');
}
...
LearnositySDK.prototype.init = function (
  ...
  if (lodash.includes(["author", "items", "reports"], service)) {
  ...
  if (service === "data") {
    ...
    output = lodash.extend(securityPacket, requestObject);
  } else if (service === "assess") {
    output = requestObject;
  } else {
    output = {
      security: securityPacket,
      request: lodash.isString(requestPacket) ? requestString : requestObject,
    };
  }
